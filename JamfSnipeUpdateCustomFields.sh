...
SnipeUpdateAsset2='{"name":"'${JamfInfoArray[0]}'",'
JamfInfoArrayCounter=1
# Loop through custom fields to build payload:
for i in "${SnipeCustomFields[@]}"; do
  SnipeUpdateAsset2=$SnipeUpdateAsset2'"'$i'":"'${JamfInfoArray[$JamfInfoArrayCounter]}'",'
  JamfInfoArrayCounter=$((JamfInfoArrayCounter+1))
done

SnipeUpdateAsset2=$(echo "$SnipeUpdateAsset2" | sed 's/,$/}/')
...
if [[ -z "$SnipeId" ]]; then
  ...
  #curl -s -X POST -H "Authorization: Bearer $SnipeBearer" "$SnipeServer/hardware" -d "$SnipeNewAsset"
else
  echo "Device found in Snipe, updating record"
  # Snipe-IT API update:
  curl -s -X PUT -H "Authorization: Bearer $SnipeBearer" -H "Content-Type: application/json" "$SnipeServer/hardware/$SnipeId" -d "$SnipeUpdateAsset2"
